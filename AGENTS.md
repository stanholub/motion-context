...
Add the MCP server from Claude Code CLI:

```bash
claude mcp add-json motion-context '{"type":"stdio","command":"/absolute/path/to/motion-context/venv/bin/python","args":["/absolute/path/to/motion-context/server.py"]}'
```

You can control where this config is stored:
- project-only: add `--scope local`
- global user config: add `--scope user`
...
Then start a new Claude Code session and run a quick check:
- `Use analyze_video on /absolute/path/to/video.mp4 and summarize key actions.`
